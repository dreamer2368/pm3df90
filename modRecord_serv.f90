...
contains
	...
	subroutine buildRecord(this,nt,n,L,ng,input_dir)
		...
		call system('mkdir -p data/'//trim(this%dir))
	end subroutine
	...
	subroutine printPlasma(this,str)
		...
		if( present(str) ) then
			open(unit=300,file='data/'//trim(this%dir)//'/record'//str,status='replace')
			open(unit=301,file='data/'//this%dir//'/xp'//str//'.bin',status='replace',form='unformatted',access='stream')
			open(unit=302,file='data/'//trim(this%dir)//'/vp'//str//'.bin',status='replace',form='unformatted',access='stream')
			open(unit=303,file='data/'//trim(this%dir)//'/E'//str//'.bin',status='replace',form='unformatted',access='stream')
			open(unit=304,file='data/'//trim(this%dir)//'/rho'//str//'.bin',status='replace',form='unformatted',access='stream')
			open(unit=305,file='data/'//trim(this%dir)//'/PE'//str//'.bin',status='replace',form='unformatted',access='stream')
			open(unit=306,file='data/'//trim(this%dir)//'/KE'//str//'.bin',status='replace',form='unformatted',access='stream')
		else
			open(unit=300,file='data/'//trim(this%dir)//'/record',status='replace')
			open(unit=301,file='data/'//trim(this%dir)//'/xp.bin',status='replace',form='unformatted',access='stream')
			open(unit=302,file='data/'//trim(this%dir)//'/vp.bin',status='replace',form='unformatted',access='stream')
			...
			open(unit=305,file='data/'//trim(this%dir)//'/PE.bin',status='replace',form='unformatted',access='stream')
