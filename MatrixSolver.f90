...
contains
	...
	function Divergence(x,dx,Ng) result(y)						!Divergence with periodic BC
		...
		real(mp) :: y(size(x,1),size(x,2),size(x,3))
		integer :: i

		y=0.0_mp
		do i=2,Ng(1)-1
		y(i,:,:) = y(i,:,:) + 0.5_mp/dx(1)*( x(i+1,:,:,1) - x(i-1,:,:,1) )
		end do
		y(1,:,:) = y(1,:,:) + 0.5_mp/dx(1)*( x(2,:,:,1) - x(Ng(1),:,:,1) )
		y(Ng(1),:,:) = y(Ng(1),:,:) + 0.5_mp/dx(1)*( x(1,:,:,1) - x(Ng(1)-1,:,:,1) )

		do i=2,Ng(2)-1
		y(:,i,:) = y(:,i,:) + 0.5_mp/dx(2)*( x(:,i+1,:,2) - x(:,i-1,:,2) )
		end do
		y(:,1,:) = y(:,1,:) + 0.5_mp/dx(2)*( x(:,2,:,2) - x(:,Ng(2),:,2) )
		y(:,Ng(2),:) = y(:,Ng(2),:) + 0.5_mp/dx(2)*( x(:,1,:,2) - x(:,Ng(2)-1,:,2) )

		do i=2,Ng(3)-1
		y(:,:,i) = y(:,:,i) + 0.5_mp/dx(3)*( x(:,:,i+1,3) - x(:,:,i-1,3) )
		end do
		y(:,:,1) = y(:,:,1) + 0.5_mp/dx(3)*( x(:,:,2,3) - x(:,:,Ng(3),3) )
		y(:,:,Ng(3)) = y(:,:,Ng(3)) + 0.5_mp/dx(3)*( x(:,:,1,3) - x(:,:,Ng(3)-1,3) )
	end function
...
!=============Not Applicable yet. Anti-aliasing for point charge should be done.===========================

	subroutine FFTAdj_Efield(Es,rhos,W,D)
